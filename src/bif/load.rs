...
pub fn prepare_loading_2(_vm: &vm::Machine, process: &RcProcess, args: &[Term]) -> BifResult {
    ...
    args[1]
        ...
        .and_then(|bytes|
                  ...
                  .map(|module| Term::boxed(heap, value::BOXED_MODULE, Box::into_raw(Box::new(module))))
                  .or_else(|_| Ok(tup2!(heap, atom!(ERROR), atom!(BADFILE))))
        )
}

pub fn has_prepared_code_on_load_1(_vm: &vm::Machine, _process: &RcProcess, args: &[Term]) -> BifResult {
...
pub fn finish_loading_1(vm: &vm::Machine, _process: &RcProcess, args: &[Term]) -> BifResult {
    let cons = match args[0].try_into() {
        Ok(cons) => { let cons: &value::Cons = cons; cons }
